...
| Erizo de Púas | 3 | 15 | 6 | 2 | 12 | **Salta** cada ~2s y va acorazado de púas: **pisarlo no lo mata** — te pincha un **10% de tu vida máxima** (la defensa no lo rebaja) y te rebota con tregua. Éntrale por un lado y gana el duelo | **Nunca** — pincha |
| Hoverbot | 4 | 16 | 6 | 3 | 14 | **Vuela** en onda senoidal | Lv5 |
| Magnetita | 5 | 22 | 8 | 5 | 18 | Lenta pero con mucha DEF — usa la Habilidad. Pisarla con nivel la mata, pero su campo se descarga y **te repele en diagonal** (rumbo aleatorio; el empujón decae solo y se corrige pulsando la dirección contraria — ojo cerca de huecos) | Lv6 — con retroceso |
...
El nivel del **hielo resbaladizo** (ver Peligros): todo el suelo tiene inercia — carrerilla que supera tu velocidad normal y se conserva al saltar. Primeros huecos que premian el doble salto; erizos que saltan: **ya no se dejan pisar** (pinchan) — esquívalos o entra al duelo por un lado.
...
Al completar un reto, el botón **«Retar a un amigo con este tiempo»** comparte una URL con un token (`?duelo=...`) que codifica **la fecha del reto, tu tiempo, tu nombre** (se pide una vez, opcional) **y tu ruta grabada**. Quien la abra verá un botón de duelo en el menú y jugará **exactamente el mismo desafío** — el reto es determinista por fecha, así que el enlace reproduce ese nivel, piloto, dificultad y azar aunque se abra días después — contra tu **fantasma**: una silueta translúcida de tu piloto que **reproduce tu recorrido real** (posición muestreada cada 12 frames): tus saltos, tus caídas, tus muertes... y **tus pausas de combate** — clavado donde te quedaste tú, con un **⚔ pulsante y la etiqueta «en duelo»** para que no parezca que se ha colgado — y una estela que delata por dónde vino y un delta en el HUD (`−1.2s` verde / `+2.3s` rojo) según vaya la carrera.

- Al terminar: veredicto (**¡DUELO GANADO!** / **DUELO PERDIDO** y por cuántos segundos) y botón de **revancha** con tu nuevo tiempo. El texto de compartir incluye el veredicto — no es el genérico del reto.
...
- La ruta viaja comprimida (cuantizada a 4px, ≤600 puntos): una partida de minuto y medio cabe en ~1.200 caracteres de URL. El acortador propio (repo `link-shortener`, desplegado en `s.enri.me`) convierte el enlace compartido en uno corto (`s.enri.me/astroleap/Xk3mP2a`); si el servicio no responde, se comparte la URL larga sin más.
