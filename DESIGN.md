...
- **No hay cámara vertical** (la cámara solo sigue en X) → los tres pisos caben en los 180px del canvas: y=150, y=105, y=60, separación 45 — deliberadamente por encima del alcance vertical del salto simple (~27 de ápice + 10 de ventana de aterrizaje), así que de piso a piso solo se sube por los peldaños de las escaleras (o pagando Energía en habilidad aérea, el atajo de siempre). Un test fija que 45 > ápice+ventana.
...
Las dos torres Extra (§2.23-2.24) vivían solo tras una URL de depuración. Ahora son el premio de un **objetivo secundario**: 12 **Cristales de Señal** (uno por nivel del mapa, `crystals` en `levels.js`, clase `SignalCrystal`), restos de la red de vigilancia de quienes construyeron al Centinela (LORE.md: «construido por alguien, hace mucho, específicamente para detener esto»). Con 5 se triangula la Torre de Vigía; con 10, la Aguja Glacial (`SIGNAL_GATES` en `game.js`).

- **Uno por nivel, umbrales 5/10 sobre 12**: los coleccionables por-nivel encajan con el patrón ♥/⚡ existente (la guía puede documentar dónde está cada uno), y dejar 2 de margen evita exigir el pleno. Al menos 7 se alcanzan con salto simple: Scrap puede abrir la Torre sin ayuda — coherente con §2.15.
...
- Un test de invariante fija la colocación: exactamente 1 por nivel del mapa, 0 en las torres, y siempre a 8-42 por encima de una plataforma (o del punto ALTO de una móvil) con solape horizontal — es decir, rozables de un salto, nunca decorativos inalcanzables.
