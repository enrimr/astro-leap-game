...
- **Acortador opcional** (`URL_SHORTENER`/`this.urlShortener` + `shortenUrl()`): las URLs con ruta rondan 1.2K caracteres — feas de compartir. Los acortadores gratuitos no sirven desde el navegador (sin CORS) y son frágiles, así que el acortado usa un backend propio (repo `link-shortener`: POST /api/shorten + 301, con lista blanca de dominios de destino), desplegado en `https://s.enri.me`. Cosmética, jamás dependencia: timeout de 2.5s y cualquier fallo comparte la URL larga; una respuesta corrupta/maliciosa (shortUrl que no sea http) tampoco cuela. Si share() pierde el gesto por la espera (Safari), se cae a copiar al portapapeles.
- **Contraseña en el servidor sin contraseña en el cliente**: el acortado libre del servicio exige `ADMIN_PASSWORD` (Bearer), pero el juego corre en el navegador del jugador y cualquier credencial embebida en su JS sería pública. La salida es `PUBLIC_PREFIXES` en el servidor: el juego acorta SIEMPRE con `prefix: 'astroleap'` (`SHORTENER_PREFIX`), y ese prefijo está eximido de contraseña — el riesgo queda acotado como en el diseño original (destino en lista blanca + límite por IP): lo peor posible es crear enlaces hacia dominios propios bajo `/astroleap/`. Si el servidor aún no conoce el prefijo público, responde 401 y el juego degrada a la URL larga, como con cualquier otro fallo.
...
### 2.30 Métricas de uso: contadores agregados en el backend propio
...
### 2.33 Techos macizos: la única plataforma que no se atraviesa desde abajo

El motor no tiene colisión lateral ni de techo a propósito (§1.2 A) — todo se atraviesa saltando, y ese perdón es parte del control. El variant `roof` rompe la regla quirúrgicamente: en el ascenso (`vy < 0`), la cabeza se frena en seco contra su cara inferior (banda de 10, espejo de la de aterrizaje; el ascenso máximo es 4.3/frame, sin tunneling). Encima se aterriza y camina igual que en cualquier plataforma. Remaches en la cara inferior como lenguaje visual (los demás variantes marcan su rareza por arriba; este muerde por abajo) y aviso de una vez al primer cabezazo.

Para qué sirve (Mundo 4 y Extra — dificultad tardía, como pedía la idea): **quitarle a la habilidad aérea la llave maestra**. Las puertas de energía se saltaban por arriba con doble salto/vuelo/dash; con un techo encima (el «pasillo sellado» de Bóveda Sellada, la puerta final de Galería de Ecos) la puerta se cronometra o se paga — la Energía deja de comprar la salida fácil justo donde el juego debe apretar. Y en la Torre de Vigía sella el atajo de Bolt: los pisos 2-3 son macizos salvo los dos tramos donde desembocan las escaleras (que funcionan saltando A TRAVÉS del piso desde el peldaño — esos jamás pueden ser techo) y las frágiles; el serpentín pasa a ser la ruta de los cuatro pilotos.

Las dos reglas de seguridad, fijadas con tests: (1) el BFS de alcanzabilidad invalida la arista «aterrizar en un techo viniendo de justo debajo» — la ruta obligatoria nunca puede depender de un aterrizaje imposible; (2) regla de holgura: desde cualquier plataforma bajo un techo, el **salto simple pasa limpio** (cabeza en el ápice por debajo de la cara inferior, medido con la física real) — el techo castiga la habilidad aérea, nunca el salto normal de la ruta. La Aguja Glacial queda sin techos de momento: sus ascensores suben al jugador y un techo sobre su recorrido lo aplastaría — si algún día se sellan tramos, lejos de los raíles.

### 2.34 Build de escritorio: Electron como envoltorio, no como port

Primer paso hacia Steam (que distribuye ejecutables, no URLs): `desktop/main.js` abre `index.html` por `file://` en una ventana de Electron, **sin preload, sin IPC y sin tocar el motor** — el juego sigue siendo el mismo HTML/JS plano que se despliega en la web, y esa unicidad es la decisión: un solo código, dos distribuciones. Electron y no Tauri por `steamworks.js` (logros, Steam Cloud y overlay cuando toque) y porque el runtime es el mismo Chromium contra el que ya se testea con puppeteer.

La única rama de comportamiento vive en `js/game.js` y se activa por user agent (`IS_DESKTOP`), con dos consecuencias razonadas:

- **Compartir apunta a la web** (`WEB_HOME`), no a `location.href` — que en escritorio es un `file://` inútil para el rival. Los duelos a distancia siguen funcionando entre escritorio y web: el token viaja igual, el retado juega en el navegador.
- **Las métricas pasivas se apagan** (`METRICS_URL = ''`): un contador de visitas es normal en una página y telemetría a declarar en una app instalada (Steam lo pregunta explícitamente). El acortador queda encendido porque solo dispara con un gesto explícito del jugador (compartir un duelo).

Lo que el envoltorio resuelve por su cuenta: enlaces externos al navegador del sistema (`setWindowOpenHandler` + `will-navigate`; dentro de la app solo vive el juego), F11 para pantalla completa, cerrar la ventana cierra la app también en macOS (es un juego, no un editor). El empaquetado es electron-builder con lista blanca de ficheros (`index.html`, `style.css`, `js/`, `fonts/`, `desktop/`): el `.app` no arrastra ni los vídeos de gameplay ni tests ni node_modules — el peso es el runtime de Electron, no el juego. Sin firma de Apple (`identity: null`): Steam no la exige; si algún día se distribuye fuera de Steam, se firma y notariza entonces. El icono se genera con `npm run generate-app-icon` (canvas, misma paleta y trucos que el banner OG: la luna rosa, órbita cian, la nave).

Pendiente conocido para el build real de Steam: la integración Steamworks en sí (logros y Steam Cloud vía steamworks.js — necesita App ID de una cuenta de partner). El mando ya está (§2.35). Las fuentes ya viajan con el juego: subset latino autoalojado en `fonts/` (~40KB, declarado en `style.css`) — la app funciona sin red y la web deja de visitar servidores de terceros. Y el guardado ya vive en fichero: `desktop/preload.js` espeja las claves `astroLeap*` de `localStorage` a `save.json` en `userData` (muestreo cada 2s + volcado síncrono al cerrar; escritura atómica tmp+rename en el main) y las siembra de vuelta al arrancar — el juego no sabe que existe el espejo, y `save.json` es la unidad que Steam Cloud sincronizará.

### 2.35 Mando: el gamepad habla teclado

El soporte de mando (web, escritorio y de facto la puerta a Steam Deck) NO añade un sistema de input: `js/gamepad.js` sondea la Gamepad API en su propio rAF y sintetiza los mismos `KeyboardEvent` (`e.code`) que ya entiende todo el juego — el `keydown` de `setupInput()` puebla `this.keys`, el combate decide por evento, los menús navegan por foco. Cero cambios en `game.js`/`entities.js`: cualquier mecánica futura que funcione con teclado funciona con mando gratis, y viceversa. Mapeo estándar W3C: A salta/confirma, B atrás/salir, X hangar, Y árbol, Start confirma, d-pad y stick izquierdo son las flechas (zona muerta 0.35).

Tres decisiones con porqué:

- **Flancos + autorepeat de software con `e.repeat=true`** (retardo 400ms, cadencia 110ms, solo flechas): mantener recorre menús como en teclado, pero JAMÁS decide en combate — `handleInput` ya descartaba `e.repeat`, así que la regla «pulsar decide, mantener acelera» (§2.28) se hereda sin una línea nueva. Y como `this.keys` queda mantenido entre flancos, sostener A es el propulsor de Bolt y el acelerador de turnos, igual que sostener ESPACIO.
- **En plataformas el eje vertical calla**: `ArrowUp` también salta (`entities.js`), así que un stick empujado hacia arriba sería un doble salto accidental que quema Energía. La capa solo deja pasar arriba/abajo en combate, hangar, árbol y menús; al cambiar el contexto con el stick aún empujado, el `keyup` sale en ese mismo frame — fijado con test.
- **Los menús DOM se activan con `click()`**: los eventos sintéticos no disparan la activación nativa del `<button>` enfocado (privilegio de los eventos de confianza del navegador). Y si no hay nada enfocado (con ratón/táctil nunca lo hay), el primer flanco se gasta en dar foco al primer botón — el jugador ve aparecer el resaltado, no un salto misterioso al segundo elemento.

En la app de escritorio, además, `autoplay-policy=no-user-gesture-required`: la Gamepad API no cuenta como activación de usuario para Chromium, y sin el switch un jugador solo-mando no oiría nada hasta tocar el ratón. Verificado de punta a punta con un mando falso guionizado dentro del Electron real: menú → foco → JUGAR → mapa → entrar al sector.

### 2.36 «¿Te han retado?»: el duelo entra por el portapapeles

El duelo a distancia viaja en la URL (`?duelo=`, §2.27) — y eso presuponía un navegador. La app de escritorio (§2.34) no tiene barra de direcciones: pinchar un enlace de duelo abriría la versión web, fuera de tu save y de tu app. El camino de vuelta es un panel en el menú («¿Te han retado?», enlace discreto junto a Ayuda) donde se pega lo que sea que te haya llegado: la URL larga, el enlace corto de `s.enri.me`, o el token a pelo. También sirve en web cuando el enlace te llega con el juego ya abierto — pegar gana a recargar.

La única pieza con miga es el enlace corto, que no lleva el token encima: hay que seguir su redirección. En web basta con navegar a él (exactamente el flujo de haberlo pinchado). En escritorio el renderer corre en `file://` y el CORS le impide leer adónde fue a parar un fetch cross-origin, así que la redirección la sigue el proceso main de Electron (`astro-resolve-url`, expuesto por `contextBridge` como la ÚNICA capacidad del puente `astroDesktop` — nada de fs ni IPC genérico al alcance de la página; GET y no HEAD porque hay servidores que no contestan HEAD, y el cuerpo ni se lee).

El panel reutiliza todo lo existente: el token pegado pasa por el mismo `decodeDuelToken` con checksum (corrupto → mensaje de error en el propio panel, nunca un duelo a medias), y si valida se asigna a `pendingDuel` y se reconstruye el menú — la misma tarjeta «⚔️ DUELO: ganar a X» que si hubieras llegado por URL. Fijado con tests: extracción del token de las tres formas, el puente de escritorio simulado, y que la basura no arma nada.

### 2.37 El último dispositivo manda: mando virtual y nombres de tecla

Los controles en pantalla (◀ ▶ SALTO, botones de combate) eran «siempre visibles: ratón, táctil o teclado, da igual». Con mando en la ecuación eso ya no se sostiene — en un salón con gamepad son pantalla perdida y huelen a port de móvil. La regla nueva: **el último dispositivo usado decide**. Un rastreador (`inputDevice` en `game.js`) escucha los cuatro orígenes — teclado, puntero, táctil y mando — y teclado o mando esconden el mando virtual (`body.no-touch-ui`); un toque o un clic lo traen de vuelta al instante, porque los jugadores de ratón SÍ usan esos botones.
...
### 2.44 Identidad audiovisual por mundo: fondos con parallax, atrezzo y un loop musical por mundo

Los cuatro mundos se distinguían por el color del rectángulo de plataforma (violeta/hielo/metal) y nada más: el 70% de cada pantalla era espacio negro, y Luna Cenizal y Núcleo Expuesto se sentían el mismo nivel con otro tinte. El criterio de salida de esta revisión: **que seis capturas de seis niveles se identifiquen por mundo sin leer el rótulo**. Todo vive en `js/scenery.js` (cargado entre `entities.js` y `levels.js`, expuesto como `window.SCENERY` — mismo patrón que `window.SFX`) y es PRESENTACIÓN pura: nada toca físicas, colisiones ni el BFS de los tests.

**Fondos con parallax, dos capas por mundo.** Cielo con gradiente propio (siempre desembocando en el mismo negro base, para no tocar el contraste del HUD ni del suelo) y dos capas de siluetas — lejana (parallax 0.12) y cercana (0.38, por delante de las estrellas a 0.35, como debe ser). Cada mundo cuenta su historia sin texto: la Cenizal lleva la luna rosa, crestas de cráter y **la silueta de la nave estrellada** (la premisa del juego, de fondo en su propio mundo); la Ferrosa, chatarra flotante, pilas de restos y mástiles con baliza; la Estación, vigas, ventanales rotos con estrellas dentro y cables colgando de un techo que ya no existe; el Núcleo, trazas de circuito, nodos hexagonales y el resplandor del núcleo respirando. Hay un quinto tema, `glacial` (picos y aurora), para los niveles de hielo.

**Determinista, no aleatorio.** La composición de cada fondo sale de un generador sembrado con el nombre del nivel (la variante mulberry32 de siempre), compuesta UNA vez por carga (`SCENERY.prepare` en `loadLevel`): el mismo nivel se ve igual en cada partida, y los mapas de la guía dan diffs limpios entre ejecuciones. Los mapas de la guía renderizan el nivel entero sin cámara, donde el span comprimido del parallax apelotonaría el fondo a la izquierda — de ahí el parámetro `flatWidth` de `prepare()` (y el `viewW` del culling de `drawLayers`).

**Atrezzo con reglas de legibilidad.** Textura leve por variant (juntas y remaches en metal, vetas en hielo, moteado en roca) y props por mundo (cristales, brotes, antenas, tuberías, cajas, nodos...) sobre las plataformas — pero solo en **losas grandes (h≥10, w≥30) de variants neutros**. Dos reglas duras: (1) los variants mecánicos (frágil, refuerzo, cinta, techo) y las piedras de paso quedan LIMPIOS — su dibujo es señal, no adorno; (2) el atrezzo es estático, sin glow y sin bob — nada puede confundirse con un pickup (esos brillan y flotan). Se cachea por plataforma (`p._decor`) y el tema viaja en `p.theme`, asignado por `loadLevel` — un `Platform` creado a pelo (tests, scripts) se dibuja como siempre.

**El combate hereda el cielo del nivel** (`SCENERY.setCurrent` en `loadLevel`, leído por `CombatSystem.draw` vía `window.SCENERY`): cielo + capa lejana quieta bajo un velo oscuro (0.55) que mantiene el contraste de retratos y texto. El mundo se reconoce también en el duelo, sin costarle legibilidad.

**Música: de 2 a 6 loops.** Menú/mapa y Mundo 1 comparten el pad ambiental de siempre (el arte del menú ES la luna rosa del Mundo 1 — y al compartir nombre de pista, pasar del mapa a un nivel del Mundo 1 no reinicia el loop). Nuevos: Ferrosa (pulso metálico sincopado en cuadrada), Estación (dron grave y pings de sonar con roce de segunda menor — una estación muerta no tiene ritmo, tiene ecos), Núcleo (arpegio insistente en menor armónica) y **jefe** (riff pesado en sierra con tritono, para los cuatro — `SFX.music.playCombat(esJefe)`). Todos en la órbita de La menor: las transiciones nivel↔combate no chocan. El cambio de loop vive en los puntos de ENTRADA (entrar a nivel, volver al mapa, ganar/huir de un duelo, respawn — `Game.playExploreMusic()`), no en `loadLevel`, que también corre en el arranque de página y tras un Game Over, donde arrancar música sin gesto del usuario sería ruido.

**Claves de presentación en `levels.js`**: `theme` (la Aguja Glacial y Grietas de Hielo eligen 'glacial' — es EL nivel de hielo, la luna rosa de fondo mentía) y `music` (la Aguja lleva el loop tenso del Mundo 2). Solo presentación: el BFS y las físicas las ignoran.

**Lo que NO se tocó, a propósito**: el jugador y los enemigos. Los blobs con ojos funcionan si el mundo alrededor está vestido — y su silueta+glow saturado es ahora EL contraste que los separa del fondo (por eso el atrezzo no lleva glow). `reduceEffects` se respeta en todo: sin pulsos (núcleo, balizas) — las siluetas son estáticas de por sí.

### 2.45 Scroll vertical: la cámara aprende a subir, y la Torre de Vigía crece a cinco pisos

Los 12 niveles del mapa viven en los 180px lógicos del canvas y la cámara solo seguía en X — la Torre de Vigía (§2.23) nació apretando tres pisos en una pantalla precisamente por esa restricción. Esta revisión la levanta de forma QUIRÚRGICA: los niveles pueden declarar `height` (y `spawnY`), y solo entonces existe cámara vertical. En los 12 niveles del mapa `cameraY` es SIEMPRE 0 y su render queda idéntico byte a byte al de antes.

**El motor, en cuatro piezas chicas:**
- **Cámara Y con lerp** (`Game.cameraTargetY`/`updateCameraY`): encuadra al jugador con el 55% de pantalla por encima (se escala hacia arriba: se mira hacia arriba), acotada a [0, height−180], con suavizado 0.12 — un salto mece la cámara, no la zarandea — y arrancando YA encuadrada al spawn en `loadLevel` (sin panorámica desde el cielo).
- **Un único translate vertical** en `Game.draw` cubre el mundo entero (fondo, plataformas, pickups, enemigos, jugador): ningún `draw(ctx, cx)` cambió de firma. Los overlays de PANTALLA (tormenta, barrido, muro, fantasma del duelo, HUD) quedan fuera del translate — y por eso un test de invariante fija que los niveles altos no llevan `ionStorm`/`sentinelWatch`/`forcedScroll` y están fuera del pool del Reto Diario (la ruta del fantasma además cuantiza y en 6 bits: máx 252).
- **Plano de muerte por nivel** (`Player.fallLimit`): caer "al vacío" era `y > 180`; en una torre de 296, 180 es el tercer piso. `loadLevel` lo fija a la altura real del nivel.
- **El cielo acompaña**: las estrellas se desplazan con el mismo parallax en Y (con wrap de pantalla: fluyen sin agotarse) y el fondo de §2.44 se ancla al PIE del nivel (`yOff` en `drawLayers`): en la cumbre solo queda cielo y estrellas — has subido por encima del paisaje, y se nota.

**La Torre a cinco pisos** replica el patrón GEOMÉTRICO probado del de tres — misma separación de 58 (que el doble salto no sube), mismos peldaños de 29, mismos aterrizajes no-techo donde desembocan, mismas frágiles dobles (ahora en los pisos 2 Y 4), mismos suelos de recogida bajo cada hueco — porque cada uno de esos números está fijado por un test que costó calibrar (§2.23). La dificultad nueva no viene de romper reglas sino del propio scroll: **ya no ves la torre entera**, el piso del que caes desaparece por arriba de la pantalla, y la re-subida se paga en cronómetro. Los tests de la torre eran paramétricos (miden la física real contra la geometría, no valores cableados) y validaron el diseño nuevo sin tocarlos; se añadieron cinco más: encuadre y convergencia de la cámara, plano de muerte por nivel, el invariante de "altos = Extra sin overlays", y que los CINCO saltos de piso mantienen el 58 con su peldaño partiéndolos en dos tramos de 29.

**La Aguja Glacial queda fuera a propósito**: su gating de meta se comprueba tramo a tramo contra el alcance del deslizón (~70) y sus ascensores están al límite exacto de columna y recorrido — estirarla es un rediseño completo, no una extensión. Si el scroll vertical demuestra que pide más, la Aguja (o una tercera torre) es la continuación natural.

### 2.46 Maniobras de la fauna: los duelos de morralla dejan de resolverse en piloto automático

La IA de los enemigos normales era una línea: siempre atacan (§1.2 D — "toda la profundidad táctica vive en las 4 opciones del jugador"). Funcionaba de tutorial, pero a mitad de partida el duelo contra morralla era un trámite idéntico cien veces — la prueba de cargo era el propio acelerador de turnos (§2.28), un botón de "sáltate esto". La salida NO es darle al jugador más teclas (el menú de 4 acciones está bien): es que **el enemigo tome decisiones que cambien cuál de tus 4 teclas es la buena** — el modelo Pokémon.

**Una maniobra por tipo, desde su identidad de plataformas** (nada inventado: cada poder de duelo es el mismo carácter que ya tenían fuera de él):

- **Dron: ninguna, a propósito.** Es el enemigo tutorial — su simpleza es la línea base contra la que las maniobras de los demás SE NOTAN.
- **Reptante — se repliega** (cada 3er turno): tu próximo golpe hace la mitad. La maniobra más simple enseña la regla general: los enemigos también defienden, y pegar en el turno malo es tirar medio turno.
- **Erizo — se eriza** (cada 3er turno): ATACAR te pincha un 10% de tu vida máxima — el golpe entra, pero cobra peaje. La MISMA regla que sus púas en plataformas (porcentual, ignora defensa, §2.32), ahora dentro del duelo. La Habilidad es una descarga a distancia: cero peaje — su primera ventaja no numérica.
- **Hoverbot — esquiva el ataque básico un 30%** (pasivo, RNG sembrado). La joya del set: la Habilidad "sin azar" existía desde el día uno como daño fiable, y contra los voladores por fin significa algo táctico — no falla NUNCA. Atacar a un volador es jugártela; pagar 3 EN es comprarte la certeza.
- **Magnetita — se blinda** (cada 3er turno): su defensa cuenta doble hasta su próximo turno. Ya era el tanque anti-ataque-básico; ahora además te hace RACIONAR: gastar la Habilidad contra el blindaje es desperdiciarla.
- **Espectro Iónico — drena 1 EN por golpe**, reutilizando EXACTAMENTE el mecanismo de los jefes (§2.41, `drain` en ENEMY_STATS: una propiedad, cero código nuevo). Es iónico: se come tu reactor. Y Defender lo blinda, como contra los jefes — coherencia total.

**Reglas transversales**: toda postura se ANUNCIA en el turno del enemigo y muerde en tu siguiente acción (telegrafiado antes de importar — leer el mensaje pasa a ser parte del duelo); dura exactamente esa ventana (caduca al empezar su siguiente turno); el repliegue y el blindaje afectan a Atacar Y a Habilidad (la postura dice "este turno tu daño no compensa", no "pulsa la otra tecla"); y todo el azar nuevo va por `RNG()` — en el Reto Diario la esquiva del Hoverbot falla igual para todo el mundo. El contraataque del Erizo puede matarte en TU propio turno — caso que nadie más comprobaba (el chequeo de derrota corría solo tras el turno enemigo) y ahora cierra el duelo limpiamente.

**Los jefes no cambian**: sus patrones (§2.12, §2.41) ya eran la versión mayúscula de esta idea. De propina, sus verbos especiales («¡Descarga corrupta!», «ignora tus defensas»...) pasan de español cableado a sus claves i18n — existían desde §2.43 pero nadie las llamaba: un jugador japonés leía castellano en mitad del clímax.

**Deuda asumida y documentada**: las maniobras alteran el consumo del RNG sembrado, así que un tiempo de Reto Diario anterior a este cambio no es comparable al milisegundo con uno posterior (el duelo contra un fantasma viejo sigue funcionando — el fantasma es un replay de posiciones, no una re-simulación). Se decidió NO cortar por fecha como en §2.40: mantener dos IAs de combate para siempre por la pureza de duelos amistosos que caducan en días no vale su peso en código.
