...
**Por qué "antes del nivel" y no en caliente**: los 4 comparten stats de combate/nivel/XP (es un único "jugador" con el cuerpo intercambiado, no 4 personajes con progreso separado) — cambiar de piloto en el mapa no resetea nada. Un selector de 4 iconos se dibuja directamente en el mapa estelar (`Game.drawCharacterSelect()`), con `Q`/`E` para ciclar entre los desbloqueados o tocando el icono; los bloqueados salen atenuados con un `?`. `game.charIconRects` guarda las cajas del último frame dibujado para que el hit-test táctil (`mapTap` en `setupTouchControls`) sepa qué se ha tocado.
